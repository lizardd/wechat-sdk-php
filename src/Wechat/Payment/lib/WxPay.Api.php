...
class WxPayApi
{
	...
	public function unifiedOrder(WxPayUnifiedOrder $inputObj,$timeOut = 6)
	{
		...
		if(!$inputObj->IsNotify_urlSet()){
			$inputObj->SetNotify_url($this->wxPayConfig['NOTIFY_URL']);//异步通知url
		}
...
//		print_r($response);
	...
	public function notify($callback, &$msg)
	{
		...
		//如果返回成功则验证签名
		try {
			$result = \WxPayResults::Init($xml,$this->wxPayConfig);
	...
	{
		//如果不需要上报数据
		if($this->wxPayConfig['REPORT_LEVENL'] == 0){
			return;
		} 
		//如果仅失败上报
		if($this->wxPayConfig['REPORT_LEVENL'] == 1 &&
	...
	{		
		...
		if (array_key_exists('DEV_MODE',$this->wxPayConfig) && $this->wxPayConfig['DEV_MODE']){
			//非严格校验
		...
		if($useCert == true){
			...
			curl_setopt($ch,CURLOPT_SSLCERT, $this->wxPayConfig['SSLCERT_PATH']);
			curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');
			curl_setopt($ch,CURLOPT_SSLKEY, $this->wxPayConfig['SSLKEY_PATH']);
		}
		//post提交方式
